...
def verify_access_token(token: str) -> Optional[dict]:
    """
    Verifies the access token and returns the payload if valid, otherwise None.
    """
    try:
...
def verify_refresh_token(token: str) -> Optional[dict]:
    """
    Verifies the refresh token and returns the payload if valid, otherwise None.
    """
    try:
...
def create_reset_password_token(email: str, expires_delta: Optional[timedelta] = None) -> str:
    ...
    """
    to_encode = {"sub": email}
    expire = datetime.utcnow() + (expires_delta or timedelta(minutes=RESET_PASSWORD_EXPIRE_MINUTES))
    to_encode.update({"exp": expire})
