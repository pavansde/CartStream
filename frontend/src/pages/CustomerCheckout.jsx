...
export default function CustomerCheckout() {
  ...
  const handleSubmit = useCallback(
    ...
    [validateCurrentStep, clearCart, cartItems, formData.shipping, appliedCoupon, isAddingNewAddress, saveNewAddress]
  );
        ...
        <div className="space-y-4">
          {cartItems.map(({ item, quantity }) => (
            <div key={item.id} className="flex justify-between items-center bg-white rounded-xl p-4 shadow-sm">
              ...
              <div className="font-bold text-gray-900">₹{(item.price * quantity).toFixed(2)}</div>
            </div>
          ))}
        </div>
      </div>
              ...
              <div className="space-y-4 mb-6 max-h-80 overflow-y-auto">
                {cartItems.map(({ item, quantity }) => (
                  <div key={item.id} className="flex justify-between items-center bg-gray-50 rounded-xl p-4">
                    ...
                    <span className="font-black text-gray-900">₹{(item.price * quantity).toFixed(2)}</span>
                  </div>
                ))}
              </div>
