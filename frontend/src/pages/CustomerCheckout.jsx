...
//       {/* Coupon Modal - Now properly memoized */}
//       <CouponModal
...
//   // In the useMemo for price calculations - Update this part:
//   const { subtotal, shippingCost, discount, total } = useMemo(() => {
//     const calcSubtotal = Array.isArray(cartItems)
//       ? cartItems.reduce(
//         (sum, { item, quantity }) => {
//           // Use variant price if available, otherwise use item price
//           const price = item?.selected_variant?.price !== null && item?.selected_variant?.price !== undefined
...
//     if (item.selected_variant?.price !== null && item.selected_variant?.price !== undefined) {
//       return Number(item.selected_variant.price) || 0;
//     }
...
//       const variantSpecs = [];
//       if (item.selected_variant.color) variantSpecs.push(item.selected_variant.color);
//       if (item.selected_variant.size) variantSpecs.push(item.selected_variant.size);
...
//     if (!item) return '/api/placeholder/60/60';

//     // Priority 1: Selected variant image
//     if (item.selected_variant?.image_url) {
//       return `http://10.118.241.149:8000${item.selected_variant.image_url}`;
//     }

//     // Priority 2: First image from variant images array
//     if (item.selected_variant?.images && item.selected_variant.images.length > 0) {
//       return `http://10.118.241.149:8000${item.selected_variant.images[0]}`;
//     }

//     // Priority 3: Main item image
//     if (item.image_url) {
//       return `http://10.118.241.149:8000${item.image_url}`;
//     }
...
//             item_id: item.id,
//             quantity,
//             // ADD THIS - variant_id is now REQUIRED
...
//       }
//     },
//     [validateCurrentStep, clearCart, cartItems, formData.shipping, appliedCoupon, shippingCost]
//   );
...
//             // Get display price and title with variant info
//             const displayPrice = item?.selected_variant?.price !== null && item?.selected_variant?.price !== undefined
//               ? item.selected_variant.price
//               : item?.price || 0;
...
//                   </div>
//                 </div>
//                 <div className="font-bold text-gray-900">₹{(displayPrice * quantity).toFixed(2)}</div>
//               </div>
//             );
//           })}
//         </div>
...
//                         </div>
//                       </div>
//                       <span className="font-black text-gray-900 whitespace-nowrap">₹{(displayPrice * quantity).toFixed(2)}</span>
//                     </div>
//                   );
//                 })}
//               </div>
...
export default function CustomerCheckout() {
  ...
  const getDisplayImage = (cartItem) => {
    ...
    if (cartItem.variant?.image_url) {
      return `http://10.118.241.149:8000${cartItem.variant.image_url}`;
    }

    // Priority 2: First image from variant images array
    if (cartItem.variant?.images && cartItem.variant.images.length > 0) {
      return `http://10.118.241.149:8000${cartItem.variant.images[0]}`;
    }

    // Priority 3: Main item image
    if (cartItem.item?.image_url) {
      return `http://10.118.241.149:8000${cartItem.item.image_url}`;
    }
