...
export default function Login() {
  ...
  const [error, setError] = useState("");
  const [loading, setLoading] = useState(false);
  const [loginRedirected, setLoginRedirected] = useState(false); // New flag state
  ...
  // Redirect if already logged in & role is known, but only if no manual redirect happened
  useEffect(() => {
  ...
  try {
    ...
    if (res.data?.access_token && res.data?.refresh_token) {
      ...
      if (userData && Object.keys(guestCart).length > 0) {
        for (const [itemId, quantity] of Object.entries(guestCart)) {
          // Use your API function to add/update backend cart items
          await addOrUpdateCartItem({ item_id: Number(itemId), quantity }, res.data.access_token);
        }
        localStorage.removeItem("cart"); // Clear local guest cart
      }

      // Now existing redirects
      if (from === "/checkout") {
